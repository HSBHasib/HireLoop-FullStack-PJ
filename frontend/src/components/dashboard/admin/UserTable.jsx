...
const UserTable = ({ users = [] }) => {
  ...
  const handleAction = async (userId, actionValue, isBanAction = false) => {
    let updateResult;

    if (isBanAction) {
      updateResult = await updateUserStatus(userId, actionValue);
    } else {
      updateResult = await updateUserRole(userId, actionValue);
    }

    if (updateResult?.modifiedCount > 0) {
      const successMsg = isBanAction
        ? `User account has been ${actionValue ? "Suspended" : "Activated"}`
        : `User role updated to ${actionValue}`;
        ...
        >
          ...
          >
            {(item) => {
              ...
              const currentStatus = isBanned ? "suspended" : "active";
              const statusStyles = statusColorMap[currentStatus];
              ...
              return (
                <Table.Row className="border-b border-neutral-900 hover:bg-zinc-900/20 transition-colors duration-150">
                  ...
                  {/* Status Badges - 🎯 banned এর ওপর নির্ভর করে dynamic UI */}
                  <Table.Cell className="py-4">
                    <div
                    ...
                    <div className="flex items-center justify-center gap-2">
                      ...
                      {!isRecruiter && (
                        <Button
                          ...
                          className="bg-sky-500/10 border border-sky-900/30 text-sky-400 hover:bg-sky-900/30 text-xs font-bold rounded-lg h-8 px-2.5 transition-all duration-200"
                        >
                          Make Recruiter
                        </Button>
                      )}
                      ...
                      {!isAdmin && (
                        <Button
                          ...
                          className="bg-amber-500/10 border border-amber-900/30 text-amber-400 hover:bg-amber-900/30 text-xs font-bold rounded-lg h-8 px-2.5 transition-all duration-200"
                        >
                          Make Admin
                        </Button>
                      )}

                      {/* 🎯 ২. অ্যাকাউন্ট কারেন্টলি ব্যানড (banned: true) থাকলে পাস করবে false (Activate এর জন্য), আর একটিভ থাকলে true (Suspend এর জন্য) */}
                      {isBanned ? (
                        <Button
                          size="sm"
                          onClick={() => handleAction(item.key, false, true)} // userId, banned: false, isBanAction: true
                          className="bg-emerald-500/10 border border-emerald-900/30 text-emerald-400 hover:bg-emerald-900/30 text-xs font-bold rounded-lg h-8 px-2.5 transition-all duration-200"
                        >
                          Activate
                        </Button>
                      ) : (
                        <Button
                          size="sm"
                          onClick={() => handleAction(item.key, true, true)} // userId, banned: true, isBanAction: true
                          className="bg-rose-500/10 border border-rose-900/30 text-rose-400 hover:bg-rose-900/30 text-xs font-bold rounded-lg h-8 px-2.5 transition-all duration-200"
                        >
                          Suspend
                        </Button>
                      )}
