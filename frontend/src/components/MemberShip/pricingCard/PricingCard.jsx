        ...
        <Card.Header className="p-0 flex flex-col items-stretch bg-transparent border-none">
          <div className="flex justify-between items-start gap-2 mb-6">
            <div className="flex items-center gap-2.5 min-w-0">
              ...
              <Card.Title className={`text-lg font-bold tracking-tight truncate m-0 ${isTertiary ? "text-white" : "text-neutral-200"}`}>
                {planItem.name}
              </Card.Title>
            </div>
            <div className="flex items-baseline text-white shrink-0">
              <span className="text-3xl font-bold">${planItem.price}</span>
              <span className="text-white/60 text-xs ml-0.5 tracking-tight">{planItem.period}</span>
            </div>
          </div>
          <Card.Description className={`text-xs font-medium mb-5 ${isTertiary ? "text-neutral-300" : "text-neutral-400"}`}>
            {planItem.subtitle}
          </Card.Description>
        </Card.Header>
